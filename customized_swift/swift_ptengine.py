...
from swift.llm.infer.infer_engine.utils import AdapterRequest, InferStreamer, LogitsStreamer, TokensIteratorStreamer, \
...
class PtEngine(InferEngine):
    ...
    def save_hidden_state(self, info, fig_path):
        color_map = {
            1: 'blue',
            2: '#00FF00',
            3: 'orange',
        ...
        sentence_embeddings = torch.concat(self.hidden_state, dim=0)
        X = sentence_embeddings.detach().cpu().float().numpy()
        tsne = TSNE(n_components=2, perplexity=2, random_state=42)
        X_embedded = tsne.fit_transform(X)

        plt.figure(figsize=(6, 6))
        with open(fig_path + '.txt', 'w') as f:
            for (x, y), cls in zip(X_embedded, info):
                plt.scatter(x, y, c=color_map[cls])
                f.write(f"{x} {y} {cls}\n")
    ...
    def _infer_full(self,
        ...
        if 'fp' in inputs.keys() and len(inputs['fp']) != 0:
            fingerprints = torch.tensor(inputs['fp'], dtype=torch.bfloat16, device=embeddings.device)
            if len(fingerprints.shape) == 2:
                fingerprints = fingerprints.unsqueeze(dim=0)
            fp_embeddings = self.fp_model.quantize(fingerprints)
            fp_embeddings = torch.reshape(fp_embeddings,
                                          (1, int(len(inputs['fp'])), 64, 4096))
            batch_size = embeddings.shape[0]
            input_ids = input_ids[0].unsqueeze(0)
            batch_indices, token_indices = torch.where(input_ids == 151669)
            for i in range(batch_size):
                embedding_i = embeddings[i].clone().unsqueeze(0)
                mask = torch.zeros_like(embedding_i, dtype=torch.bool)
                mask[batch_indices, token_indices] = True
                embedding_i = embedding_i.masked_scatter(mask, fp_embeddings).squeeze()
                embeddings[i] = embedding_i

        generate_kwargs.pop('input_ids', None)
        generate_kwargs.pop('fp', None)
        generate_kwargs.pop('graph', None)
        generate_kwargs['inputs_embeds'] = embeddings
        ...
        if generation_config.output_attentions:
            l, t = len(output['attentions']), output['attentions'][0][-1].shape[-1]
            # print(l, t, len(output['attentions'][0]), output['attentions'][0][-1].shape,
            #       output['attentions'][10][-1].shape)
            attention = output['attentions'][1][-1][..., :t]
            for i in range(2, l):
                attention += output['attentions'][i][-1][..., :t]
            # print(attention.shape)
            attn_matrix = (attention)[0].mean(0).squeeze()[1:].to(torch.float).detach().cpu().numpy()

            attn_matrix = attn_matrix
            x = np.arange(len(attn_matrix))

            plt.figure(figsize=(12, 5))
            plt.bar(x, attn_matrix, color="red")
            plt.title("Vector Bar Plot")
            plt.xlabel("Index")
            plt.ylabel("Value")
            plt.xticks(np.arange(0, len(attn_matrix), 50), fontsize=6)

            plt.savefig(f"./vector/vector_nepp_{self.cnt}.png", dpi=300, bbox_inches='tight')
            with open("./vector/v.txt", 'w') as f:
                for idx, attn in enumerate(attn_matrix):
                    f.write(f"{idx} {attn}\n")
            self.cnt += 1
            plt.close()
        ...
        if generation_config.output_hidden_states:
            ...
            last_layer_steps = [step_data[-1] for step_data in hidden_states[1:]]
            n = len(last_layer_steps)
            last_layer_steps = torch.concat(last_layer_steps, dim=1)
            sentence_embeddings = torch.sum(last_layer_steps, dim=1) / n  # (1, 4096)
            print(sentence_embeddings.shape)
            self.hidden_state.append(sentence_embeddings)
    ...
    def _infer(
        ...
        if len(batched_inputs) > 0:
            ...
            # print(template_inputs[0].keys())
            # exit()
            if 'fp' in template_inputs[0].messages[0].keys():
                inputs['fp'] = template_inputs[0].messages[0]['fp']  # 添加了fp信息
            if 'graph' in template_inputs[0].messages[0].keys():
                inputs['graph'] = template_inputs[0].messages[0]['graph']  # TODO 添加了graph信息
            # if 'SMILES' in template_inputs[0].messages[0].keys() and 'fp' not in template_inputs[0].messages[0].keys():
            #     inputs['fp'] = get_fp(template_inputs[0].messages[0]['SMILES'])
            #     print(inputs['fp'])
