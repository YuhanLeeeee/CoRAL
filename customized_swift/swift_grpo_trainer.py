...
from trl.trainer import grpo_trainer, ppo_trainer
...
from .swift_plugin import MultiTurnScheduler
...
from swift.trainers.rlhf_trainer.rlhf_mixin import RLHFTrainerMixin
from swift.trainers.rlhf_trainer.utils import (_ForwardRedirection,
                                               load_pil_img,
                                               patch_lora_merge, patch_lora_unmerge,
...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(self,
        ...
        if self.use_vllm:
            ...
            from .swift_ptengine import PtEngine
    ...
    def set_fp_model(self, fp_model):
        self.engine.set_fp_model(fp_model)
    ...
    def _infer(self,
        ...
        if self.vllm_mode == 'server':
            ...
            # TODO inference and return response
            results: List[ChatCompletionResponse] = self._engine_infer(
    ...
    def _dynamic_sampling(self, inputs, rewards, rewards_per_func, completions):
        ...
        while resample_count < self.args.max_resample_times:  # 进行动态采样
            grouped_rewards = rewards.view(-1, self.num_generations)  # 奖励值
            group_std = grouped_rewards.std(dim=1)  # 奖励方差

            valid_mask = (group_std > 0).repeat_interleave(self.num_generations)  # 有效mask
            all_inputs = gather_object(inputs)
            valid_samples.extend([inp for inp, mask in zip(all_inputs, valid_mask) if mask])  # 增多有效样本
            ...
            if len(valid_samples) >= self.args.generation_batch_size:  # 足够多的有效样本
                break
    ...
    def _compute_loss(self, model, inputs):
        ...
        if self.loss_type == 'grpo':
            ...
            # case masks
            low_mask = (ratio < (1 - self.epsilon_low)) & (advantages < 0)
            high_mask = (ratio > (1 + self.epsilon_high)) & (advantages > 0)
            other_mask = ~(low_mask | high_mask)

            beta1 = 0.25  # gppo_loss_beta1
            beta2 = 1.0  # gppo_loss_beta2

            # three cases
            clip_pg_losses1 = torch.zeros_like(advantages)
            clip_pg_losses1[low_mask] = -beta1 * (1 - self.epsilon_low) / ratio_detached[low_mask] * ratio[low_mask] * \
                                        advantages[low_mask]
            clip_pg_losses1[high_mask] = -beta2 * (1 + self.epsilon_high) / ratio_detached[high_mask] * ratio[high_mask] * \
                                         advantages[high_mask]
        ...
        with patch_profiling_context(self, 'generate'):
            if self.vllm_mode == 'server':
                ...
                for request in infer_requests:
                    ...
                    # if 'SMILES' in request.keys():
                    #     request_dict['fp'] = get_fp(request['SMILES'])
                    new_infer_requests.append(request_dict)
